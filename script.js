var palabras = ["teclado", "bulbasour", "pantalla", "telefono"];

var tamanoPalabra = 60;
var espacio = 10;
...
function dibujarHombre(fallos){
    switch (fallos) {
        case 1:                    
            pincel.fillRect(300, 550, 200, 10);
            break;
        case 2:                    
            pincel.fillRect(450, 0, 10, 550);
            break;
        case 3:                    
...
function ganar(aciertos,palabraAleat){
    if (aciertos == palabraAleat.length) {
        ...
        pincel.fillText("Felicidades, ganaste!", 800, 400);
...
function perder(fallos){
    if (fallos == 10) {
        ...
        pincel.font = "30px Arial";
        pincel.fillText("Perdiste :c", 800, 400);
        document.removeEventListener('keypress',pantallaJuego , false);
    }
...
function indicarLetras(palabraAleat) {
    ...
    for (var i = 0; i < palabraAleat.length; i++) {
        pincel.resetTransform();
        pincel.fillRect(posX+((tamanoPalabra*i)+espacio*i),650,tamanoPalabra,10);        
    }   
}
        ...
        if (nonuevaLetra == false) {
            ...
            if (indices.length != 0) {
                for (var i = 0; i < indices.length; i++) {
                    ...
                    pincel.fillText(palabraAleat[indices[i]].toUpperCase(), (tamanoPalabra/5) + posX + ((tamanoPalabra*indices[i]) + espacio*indices[i]), 630);
                }
            }
            else{
                fallos+=1;
                ...
                pincel.fillStyle = '#0e3310';
                pincel.font = "60px Arial";
                pincel.fillText(letra.toUpperCase(), (tamanoPalabra/5) + posX + ((tamanoPalabra*(fallos-1)) + espacio*(fallos-1)), 750);
